...
from knimin.lib.sql_connection import SQLConnectionHandler, Transaction, TRN


DB_CREATE_TEST_TABLE = """CREATE TABLE pm.test_table (
    str_column      varchar DEFAULT 'foo' NOT NULL,
    bool_column     bool DEFAULT True NOT NULL,
    int_column      bigint NOT NULL);"""

DB_DROP_TEST_TABLE = """DROP TABLE IF EXISTS pm.test_table;"""
...
class TestBase(TestCase):
    def setUp(self):
        ...
        with connect(user=config.db_user, password=config.db_password,
                     host=config.db_host, port=config.db_port,
                     database=config.db_database) as self.con:
    ...
    def _assert_sql_equal(self, exp):
        ...
        with connect(user=config.db_user, password=config.db_password,
                     host=config.db_host, port=config.db_port,
                     database=config.db_database) as con:
            with con.cursor() as cur:
                cur.execute("SELECT * FROM pm.test_table")
                obs = cur.fetchall()
            con.commit()
...
class TestConnHandler(TestBase):
    ...
    def test_sql_executor_no_sql_args(self):
        sql = "INSERT INTO pm.test_table (int_column) VALUES (1)"
    ...
    def test_sql_executor_with_sql_args(self):
        sql = "INSERT INTO pm.test_table (int_column) VALUES (%s)"
    ...
    def test_execute_fetchone_no_sql_args(self):
        ...
        sql = "SELECT str_column FROM pm.test_table WHERE int_column = 1"
    ...
    def test_execute_fetchall_no_sql_args(self):
        self._populate_test_table()
        sql = "SELECT * FROM pm.test_table WHERE bool_column = False"
        obs = self.conn_handler.execute_fetchall(sql)
        self.assertEqual(obs, [['test3', False, 3], ['test4', False, 4]])

    def test_execute_fetchall_with_sql_args(self):
        self._populate_test_table()
        sql = "SELECT * FROM pm.test_table WHERE bool_column = %s"
...
class TestTransaction(TestBase):
    ...
    def test_add(self):
        with TRN:
            ...
            sql1 = "INSERT INTO pm.test_table (bool_column) VALUES (%s)"
            args1 = [True]
            TRN.add(sql1, args1)
            sql2 = "INSERT INTO pm.test_table (int_column) VALUES (1)"
            TRN.add(sql2)
            args3 = (False,)
            TRN.add(sql1, args3)
            sql3 = "INSERT INTO pm.test_table (int_column) VALEUS (%(foo)s)"
            args4 = {'foo': 1}
            TRN.add(sql3, args4)
    ...
    def test_execute_return(self):
        with TRN:
            ...
            TRN.add(sql, ['test_insert', 2])
            sql = """UPDATE pm.test_table SET bool_column = %s
    ...
    def test_execute_huge_transaction(self):
        with TRN:
            ...
            sql = """INSERT INTO pm.table_to_make (the_trans_to_fail)
                     VALUES (1)"""
            TRN.add(sql)
    ...
    def test_execute_fetchlast(self):
        with TRN:
            ...
            sql = """SELECT EXISTS(
                        SELECT * FROM pm.test_table WHERE int_column=%s)"""
            TRN.add(sql, [2])
    ...
    def test_execute_fetchflatten(self):
        with TRN:
            ...
            sql = "SELECT str_column, int_column FROM pm.test_table"
            TRN.add(sql)

            sql = "SELECT int_column FROM pm.test_table"
            TRN.add(sql)
